...
let () =
  ...
  Arg.parse
    ...
    arg usage ;
  if !files_path = "" then ( Printf.printf "%s\n" usage ; exit 1 ) else ()
...
let send_reply h reply =
  ...
  Duppy.Monad.bind (write http_header) (fun () ->
      ...
      | File fd ->
          ...
          Duppy.Monad.bind
            (Duppy.Monad.Io.write_bigarray ?timeout:None ~priority:Non_blocking
               h ba) (fun () -> Duppy.Monad.return (close ()) )
...
let file_request path _ request =
  ...
  in
  let __pa_duppy_0 =
    index_uri path "index.html" request.request_protocol uri
  in
...
let file_handler =
  ((fun _ -> Duppy.Monad.return true), file_request !files_path)
...
let () =
  ...
  Duppy.Task.add scheduler
    {Duppy.Task.priority= Non_blocking; events= [`Read sock]; handler= incoming} ;
